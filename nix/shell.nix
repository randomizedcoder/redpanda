{
  ...
  git,
  zstd,
  # Static-analysis / linting tooling. Pinned via the flake so every
  # contributor (and CI) runs identical versions — matches the project's
  # pedantic posture on build reproducibility.
  go,
  golangci-lint,
  shellcheck,
  shfmt,
...
mkShell {
  packages = [
    ...
    git
    zstd
    # Static analysis — see note on input list above.
    go
    golangci-lint
    shellcheck
    shfmt
  ...
  shellHook = ''
        echo -e "\033[31m"
        # Heredoc terminator (LOGO) must sit at column 0 post Nix indent
        # stripping so bash recognises it; indented terminators do not close
        # the heredoc and would silently swallow the .bazelrc.nix block below.
        cat <<'LOGO'
            ____           __                      __
