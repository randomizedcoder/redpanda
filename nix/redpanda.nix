{
  ...
  # Extra Bazel targets to build alongside the redpanda binary. Each
  # entry is a Bazel label (e.g. "//src/v/net/tests:uds_bench_rpbench").
  # Outputs are installed into $out/libexec by `extraInstallBins`.
  extraBazelTargets ? [ ],
  # List of {src, dst} pairs to install from bazel-bin after the build.
  # `src` is the path under bazel-bin (e.g. "src/v/net/tests/uds_bench_rpbench");
  # `dst` is the path under $out (e.g. "libexec/uds_bench_rpbench"). Kept
  # separate from the redpanda binary so bench tooling doesn't collide
  # with the main $out/bin/redpanda install.
  extraInstallBins ? [ ],
}:

assert builtins.elem optimizationLevel [
  "default"
  "release"
  "lto"
];
assert builtins.elem pgoMode [
  "off"
  "instrument"
  "optimize"
];
...
let
  ...
  bazelPlatformBin =
    ...
    in
    "${bazel}/bin/bazel-8.5.0-${os}-${arch}";
  targets = [ "//src/v/redpanda:redpanda" ] ++ extraBazelTargets;

  # Optimization level flags (orthogonal to PGO).
  # See .bazelrc: --config=release (opt + secure), --config=lto (ThinLTO).
  optimizationArgs =
    lib.optionals (optimizationLevel == "release") [
      "--config=release"
    ]
  ...
  pgoArgs =
    lib.optionals (pgoMode == "instrument") [
  ...
  interpreterSedScript =
    ...
    lib.concatStringsSep "\n" (
      map (name: "/^#!.*${name}/{s|^#!.*|#!${interpreterMap.${name}}|;}") orderedNames
    );
  ...
  cargoBazel =
    runCommand "cargo-bazel-patched"
      {
  ...
  goProxyCache = runCommand "go-proxy-cache" { } ''
    ...
    ln -s ${
      fetchurl {
        url = "https://proxy.golang.org/google.golang.org/protobuf/@v/v1.36.11.info";
        sha256 = "2156715d128777c2a6fae6107d12a0ab60c2b1deed4fba5a2b2481c68911082a";
      }
    } $out/google.golang.org/protobuf/@v/v1.36.11.info
    ln -s ${
      fetchurl {
        url = "https://proxy.golang.org/google.golang.org/protobuf/@v/v1.36.11.mod";
        sha256 = "a75c105a852fbd8da8d8cfac09c2eab9a206cfd27ed37c973737e23f632ca96e";
      }
    } $out/google.golang.org/protobuf/@v/v1.36.11.mod
    ln -s ${
      fetchurl {
    ...
    } $out/google.golang.org/protobuf/@v/v1.36.11.zip
    echo '{"Version":"v1.36.11"}' > $out/google.golang.org/protobuf/@v/list

    mkdir -p $out/github.com/golang/protobuf/@v
    ln -s ${
      fetchurl {
        url = "https://proxy.golang.org/github.com/golang/protobuf/@v/v1.5.4.info";
        sha256 = "840270c813a1c9b8cfe1b66d534336c71dad9da2e1c57c9df3743aaa5eaca219";
      }
    } $out/github.com/golang/protobuf/@v/v1.5.4.info
    ln -s ${
      fetchurl {
        url = "https://proxy.golang.org/github.com/golang/protobuf/@v/v1.5.4.mod";
        sha256 = "c5f873c621cfaaf563f8b66a0501a5be14390cb0859e5187ce616d0312a6c8f8";
      }
    } $out/github.com/golang/protobuf/@v/v1.5.4.mod
    ln -s ${
      fetchurl {
        url = "https://proxy.golang.org/github.com/golang/protobuf/@v/v1.5.4.zip";
        sha256 = "9a2f43d3eac8ceda506ebbeb4f229254b87235ce90346692a0e233614182190b";
      }
    } $out/github.com/golang/protobuf/@v/v1.5.4.zip
  ...
  bazelrcNix = ''
    ...
    build --host_action_env=ACLOCAL_PATH=${
      lib.concatStringsSep ":" [
        "${automake}/share/aclocal"
        "${libtool}/share/aclocal"
        "${pkg-config}/share/aclocal"
...
stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    ${lib.optionalString (bazelCacheDir != "") ''
            ...
            # touch updates mtime on existing files; only chmod if we created it.
            # chmod on a file owned by a different nixbld user (from a prior build)
            # fails with EPERM even when the file is already group-writable from
            # the parent dir's setgid bit.
            if [ ! -e "${bazelCacheDir}/output_base/lock" ]; then
              touch "${bazelCacheDir}/output_base/lock"
              chmod g+w "${bazelCacheDir}/output_base/lock"
            fi
            # Bazel creates a per-user `server/` subdir with mode 700 inside
            # output_base. When a subsequent build runs as a different nixbld
            # user, that user cannot enter or remove the dir (even with group
            # write on the parent), and Bazel's startup fails with
            # "server directory could not be created". Detect that situation
            # early and emit an actionable message — we cannot fix it from
            # inside the sandbox, only root can rm the stale dir.
            if [ -d "${bazelCacheDir}/output_base/server" ] \
               && ! ls "${bazelCacheDir}/output_base/server" >/dev/null 2>&1; then
              cat >&2 <<EOF
      FATAL: stale Bazel server/ dir at ${bazelCacheDir}/output_base/server
             owned by a previous nixbld user with mode 700. Remove it:
                 sudo rm -rf ${bazelCacheDir}/output_base/server
      EOF
              exit 1
            fi
    ''}
  ...
  installPhase = ''
    ...
    # Install extra binaries requested by the caller (e.g. bench tooling).
    # Each entry is {src, dst, extraLibs?}; src is relative to bazel-bin,
    # dst is relative to $out. `extraLibs` is an optional list of .so
    # paths (relative to bazel-bin) that get copied next to the dst
    # binary — $ORIGIN is added to rpath so they load without LD_LIBRARY_PATH
    # tricks. This matches the Bazel `cc_binary` runtime layout where
    # test-only dylibs (e.g. Boost `lib_internal_test.so`) live under
    # `_solib_*` and are normally resolved via runfiles.
  ''
  + lib.concatMapStrings (e: ''
    install -D -m755 bazel-bin/${e.src} $out/${e.dst}
    patchelf --add-rpath ${krb5.lib}/lib:${openssl.out}/lib $out/${e.dst}
    ${lib.concatMapStrings (l: ''
      install -D -m644 bazel-bin/${l} $out/$(dirname ${e.dst})/$(basename ${l})
    '') (e.extraLibs or [ ])}
    ${lib.optionalString ((e.extraLibs or [ ]) != [ ]) ''
      patchelf --add-rpath '$ORIGIN' $out/${e.dst}
    ''}
  '') extraInstallBins;
